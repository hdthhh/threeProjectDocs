...
onMounted(() => {
  ...
  resizefn = () => {
    if (particles.material) particles.material.uniforms.uResolution.value.set(ddd.value.offsetWidth * window.devicePixelRatio, ddd.value.offsetHeight * window.devicePixelRatio)
...
function animate() {
  ...
  if (tf && particles?.material?.uniforms?.uProgress) {
    tf = false
    add()
  }
}
...
loader.load('/threeProjectDocs/models.glb', (gltf) => {
  console.log(gltf)
  ...
  particles.material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uResolution: new THREE.Uniform(new THREE.Vector2(ddd.value.offsetWidth * window.devicePixelRatio,ddd.value.offsetHeight * window.devicePixelRatio)),
